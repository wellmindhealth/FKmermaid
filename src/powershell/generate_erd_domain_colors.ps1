...
"@
    ...
    foreach ($entity in $filteredEntities) {
        ...
        if ($lFocus -and $lFocus -ne "") {
            ...
            $focusEntitiesForValidation = $lFocus -split ',' | ForEach-Object { $_.Trim() }
            $missingEntities = @()
            $excludedEntities = @()
            
            foreach ($focusEntity in $focusEntitiesForValidation) {
            ...
            $fullFocusEntity = $focusEntitiesForValidation -join ','
        }
        
        # NEW: Use domain-based styling instead of old 5-tier system
...
"@
    ...
    foreach ($entity in $filteredEntities) {
        ...
        if ($knownTables -contains $entityName -and $entityName -and $entityName.Length -gt 0) {
            ...
            $mermaidContent += "        +string domain`n"
            $mermaidContent += "    }`n`n"
        }
    }
...
function Get-EntityStyle {
    ...
    # NEW: Domain-based styling system
    # Check for exact match first (keep existing custom styles)
    if ($cssStyles.ContainsKey($entityName)) {
        return $cssStyles[$entityName]
    }
    
    # Check for plugin-specific styling (keep existing custom styles)
...
function Get-EntityDomain {
    ...
    # Check catchall section if it exists
    # Note: catchall is at the root level, not under domains
    if ($catchallConfig -and $catchallConfig.entities) {
        foreach ($layer in $catchallConfig.entities.PSObject.Properties) {
            if ($layer.Value -contains $entityName) {
                return "catchall"
...
function Get-DomainStyleName {
    ...
    # Check if entity is directly related to focus
    # We need to construct the full entity name to match against $relatedEntities
    # The $relatedEntities array contains full names like "pathway_progMember"
    # But $entityName is just the base name like "progMember"
    # We need to find the plugin for this entity to construct the full name
    $fullEntityName = $entityName  # Default to base name
    $isDirectlyRelated = $false
    
    # Try to find the full entity name in the related entities
    foreach ($relatedEntity in $relatedEntities) {
        if ($relatedEntity -like "*_$entityName") {
            $fullEntityName = $relatedEntity
    ...
    if ($entityDomain -eq $focusDomain) {
        # Same domain
        if ($isFocusEntity) {
            return $domainPalettes[$entityDomain].focus.style      # participant_focus
        } elseif ($isDirectlyRelated) {
            return $domainPalettes[$entityDomain].related.style    # participant_related
        } else {
            return $domainPalettes[$entityDomain].other.style      # participant_other
        }
    } else {
        # Cross-domain
        if ($isDirectlyRelated) {
            return $domainPalettes[$entityDomain].related.style    # provider_related, pathway_related, etc.
        } else {
            return $domainPalettes[$entityDomain].other.style      # provider_other, pathway_other, etc.
        }
    }
}
...
if (Test-Path $domainsFile) {
    try {
        ...
        # NEW: Load domain color palettes
        $domainPalettes = @{}
        foreach ($domain in $domainsConfig.PSObject.Properties) {
            if ($domain.Value.colors) {
                $domainPalettes[$domain.Name] = $domain.Value.colors
                Write-Host "   Loaded color palette for domain: $($domain.Name)" -ForegroundColor Green
            }
        }
        Write-Host "   Loaded $($domainPalettes.Count) domain color palettes" -ForegroundColor Green
    } catch {
