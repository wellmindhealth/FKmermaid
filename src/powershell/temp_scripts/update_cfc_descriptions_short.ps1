# Update CFC Descriptions Script - Shortened Version
...
# Shortened detailed descriptions for @@Description comments
...
Business Context: progMember represents the active enrollment of a participant in a specific treatment program. This tracks a participant's journey through a structured therapeutic intervention, measured and managed throughout their treatment pathway.
...
Key Relationships: progMember links the participant (member) to their chosen treatment program, creating a one-to-many relationship that allows participants to enroll in multiple programs while maintaining separate progress tracking.
"@
...
Business Context: journal represents the personal documentation and reflection system that enables participants to create, manage, and share personal notes, reflections, and treatment-related documentation throughout their therapeutic journey.
...
Business Context: dmHTML represents the HTML content management system for website pages and dynamic content delivery that supports the platform's information architecture and user communication needs. This system manages the creation, editing, versioning, and delivery of web content.
...
Technical Role: dmNews manages the complete lifecycle of news content and announcements including content creation, publication, delivery, and integration with organizational communication frameworks. The system implements content management including targeted delivery and audience segmentation.
...
Business Context: dmFile represents the file resource management system for downloadable documents and secure digital asset storage that supports user needs, platform functionality, and organizational communication requirements. This system manages the storage, organization, versioning, and delivery of downloadable documents and resources.
...
function Update-CFCFile {
    ...
    if ($content -match 'hint\s*=\s*["''][^"'']*["'']') {
        $content = $content -replace 'hint\s*=\s*["''][^"'']*["'']', "hint=`"$ShortHint`""
    } else {
        # Add hint attribute if it doesn't exist
...
# Main execution
Write-Host "Starting CFC description updates..." -ForegroundColor Green
